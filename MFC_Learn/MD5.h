...
public:
    ...
    // 编码函数
    string encode(char* src, int len) {
    ...
    // 填充字符串
    vector<unsigned int> padding(char* src, int len) {
        // 以512位,64个字节为一组
        unsigned int remain = (len * 8) % 512 == 448? 512 : 448 - (len * 8) % 512;
        unsigned int num = ((remain + len * 8) + 64) / 512;
    ...
    string format(unsigned int num) {
        ...
        for(int i = 0; i < 4; i++) {
            ...
            for(int j = 0; j < 2; j++) {
                tmp = str16[b%16] + tmp;          //通过str16[] = "0123456789abcdef"将unsigned int 转换为八个16进制数
                b /= 16;
            }
            res += tmp;
        }
