...
typedef bool    (*PSubKey)[16][48];    //bool* p[16][48]  (定义存储了指向p[16][48]的首地址的变量的地址)  
									   //typedef bool (*PSubKey)[16][48];则是定义了一种上面所说的那个变量的类型
									   //p[16][48]是bool变量，(*PSubKey)[16][48]是bool变量，*PSubKey是p[16][48]的首地址，PSubKey是p[16][48]的首地址的地址
